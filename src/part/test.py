import pandas as pd
import numpy as np
from db import get_daily_province_in,get_daily_province_out,get_daily_place_active
...
def  test2():
    ...
    in_result = pd.DataFrame(in_result)
    out_result = get_daily_province_out(province)
    out_result = pd.DataFrame(out_result)
    date_result = get_daily_place_active(province)
    date_result = pd.DataFrame(date_result)
    result = pd.merge(in_result,out_result,on=['province','date'],how='outer') 
    result = pd.merge(result,date_result,on=['province','date'],how='outer') 
    print(in_result,'\n',out_result,'\n',date_result,'\n',result)
